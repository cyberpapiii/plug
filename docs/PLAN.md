...
- **Active upstream supervision (item 2b)** — 🔜 final PR: proactive restart/reconnect of an upstream that stays `degraded` past a threshold, building on the now-first-class `degraded` state and the per-upstream metrics
