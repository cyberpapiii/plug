...
- **`DownstreamTransport::Ipc` identity split (KTD3)** — 🔜 next PR: give IPC its own `ipc:{id}` lazy-session-key namespace + `NotificationTarget::Ipc`, replacing the `Stdio` masquerade; guarded by the parity matrix + a reconnect-ownership test.
