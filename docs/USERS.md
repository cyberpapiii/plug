...
plug is built for **individual power users** — developers and AI practitioners who use multiple AI coding and agent clients daily and are frustrated by the fragmentation of MCP server management.

plug is NOT built for:
...
**Success metric**: plug should run for weeks without intervention, automatically recovering from transient failures.
...
| S5 | As a polyglot, I want plug to generate the correct config snippet for each client (Claude Code .mcp.json, Cursor config, etc.) so that I don't have to figure out the format | 1 | P0 |
| S6 | As a tinkerer, I want to add a new MCP server with `plug servers add` and have it immediately available to all clients | 3 | P1 |
| S7 | As a server admin, I want to start plug as a daemon with `plug start` so that it runs without a terminal | 4 | P1 |
...
| ID | Story | Persona | Priority |
|----|-------|---------|----------|
| C1 | As a Cursor user, I want plug to automatically limit tools to 40 (Cursor's hard cap) and prioritize the most-used tools so that I don't lose tools silently | 1 | P0 |
| C2 | As a Windsurf user, I want plug to limit tools to 100 | 1 | P0 |
| C3 | As a VS Code user, I want plug to limit tools to 128 | 1 | P0 |
...
| C5 | As a Codex user, I want `resources/list` to always return `{resources: []}` (not an error) so that Codex doesn't mark plug as unavailable | 1 | P0 |
| C6 | As an OpenCode user, I want plug to serve SSE so that OpenCode can connect (it doesn't support Streamable HTTP yet) | 1 | P1 |
| C7 | As a Zed user, I want to connect via stdio (`plug connect`) since Zed only supports stdio | 1 | P1 |
| C8 | As a Claude Code user, I want plug to support tool search / catalog mode so that large tool sets don't bloat my context window | 1 | P1 |
...
| ID | Story | Persona | Priority |
|----|-------|---------|----------|
| R1 | As a polyglot, I want plug to keep working when one MCP server crashes — only that server's tools should be affected | 1 | P0 |
| R2 | As a server admin, I want plug to automatically reconnect to servers that come back online | 4 | P0 |
...
| R5 | As a server admin, I want plug to run for weeks without intervention | 4 | P1 |
| R6 | As a polyglot, I want no orphaned MCP server processes when plug shuts down | 1 | P0 |
...
| D2 | As a tinkerer, I want to see a real-time activity log of all MCP requests flowing through plug | 3 | P1 |
| D3 | As an automator, I want `plug status --output json` to return structured server health data | 2 | P0 |
| D4 | As a polyglot, I want `plug doctor` to diagnose common issues (port conflicts, missing env vars, expired tokens) | 1 | P1 |
...
| F3 | As a polyglot, I want config changes to apply without restarting plug (hot reload) | 1 | P1 |
| F4 | As a polyglot, I want per-client overrides (tool limits, priority tools) in the same config file | 1 | P2 |
| F5 | As a polyglot, I want config validation with clear error messages (`plug config validate`) | 1 | P1 |
...
**Without plug**: Configure each server in each client. 4 servers x 4 clients = 16 configurations across 4 different config file formats. Takes 30-60 minutes. Debugging port conflicts takes another 30 minutes.
...
**Without plug**: Each agent spawns its own MCP server instances. 5 x 3 = 15 processes. Port conflicts. Memory bloat. Some agents fail silently because servers are already bound.

**With plug**: All 5 agents connect to one plug instance. 3 server processes total. Zero conflicts. If Postgres server crashes at 4 AM, plug's circuit breaker kicks in, other tools keep working, Postgres auto-reconnects when it comes back.
...
**Without plug**: Edit 6 config files. Restart 6 clients. Cross fingers that the tool name doesn't collide with existing tools.

**With plug**:
```bash
plug servers add jira --command "npx -y @modelcontextprotocol/server-jira" --env JIRA_TOKEN=$JIRA_TOKEN
...
**Without plug**: No visibility. Cursor silently drops tools beyond 40. No error message. No way to know which tools are missing.
...
5. **Config file is edited while plug is running** — What happens? (Answer: hot-reload, add new servers, remove deleted ones)
...
8. **User runs `plug connect` from two different terminals for the same client type** — Are they isolated? (Answer: yes, each gets its own session)
9. **Server returns 10,000 tools** — What happens? (Answer: catalog mode, search meta-tool, lazy schemas)
10. **User wants to temporarily disable a server without removing it** — How? (Answer: `enabled = false` in config, or `plug servers disable <name>`)
