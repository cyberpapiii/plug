...
- daemon IPC notification push and subscription parity
...
4. **Add downstream handler methods**
   ...
   - `plug/src/ipc_proxy.rs` for daemon-backed `plug connect`

   Extend daemon IPC with a notification push path so daemon-backed clients can
   receive `notifications/resources/updated` instead of downgrading capability
   advertisement.
...
6. **Add daemon IPC notification delivery**
   Add a dedicated daemon-to-proxy notification stream so `plug connect` can
   receive MCP notifications without disturbing the existing serialized
   request/response path on the primary IPC session.

7. **Remove dead TUI dependencies and stale context**
...
8. **Keep reload semantics explicit, not magical**
...
- **API surface parity**
  resource subscriptions must work through direct stdio, HTTP, and daemon-backed
  `plug connect`.
...
- [x] direct stdio, HTTP, and daemon-backed `plug connect` all have resource subscription parity
- [x] merged capabilities advertise `resources.subscribe` truthfully
- [x] dead TUI dependencies are removed from the workspace manifest
- [x] no touched docs imply router/runtime reload behavior that the code does not implement
