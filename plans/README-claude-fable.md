...
| 013 | [OAuth + supervision e2e tests](013-oauth-supervision-e2e-tests-claude-fable.md) — expiry→refresh→call under load; HTTP upstream crash-restart | tests | M | — | TODO |
...
| 018 | [Downstream OAuth conformance spike (todo 057)](018-downstream-oauth-conformance-spike-claude-fable.md) — RFC 8414/9728 gap matrix + live probes (docs only; also owns the scope-semantics question handed over by 020) | direction | M | — | TODO |
| 019 | [HTTP session task teardown](019-http-session-task-teardown-claude-fable.md) — clean a departing HTTP session's tasks on DELETE and idle expiry (mirrors existing IPC teardown; the one asymmetric per-session map) | bug | M | — (015 also touches http/server.rs: sequence) | TODO |
| 020 | [Downstream OAuth store hardening](020-downstream-oauth-store-hardening-claude-fable.md) — eager expiry sweep + client_credentials token reuse; pins unenforced-scope behavior with a characterization test | security | M | — | TODO |
...
| 022 | [PLAN.md dispatcher truth fix](022-plan-doc-dispatcher-truth-fix-claude-fable.md) — remove the "only remaining dispatcher item"/"decomposition also remains" claims contradicted by the ✅ entries two lines below (docs only; the memory-contamination root cause) | docs | S | — | TODO |
